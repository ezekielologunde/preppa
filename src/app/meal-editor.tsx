  ...
  return (
    <View style={{ backgroundColor: CARD, borderRadius: 18, padding: 12, flexDirection: 'row', gap: 12, alignItems: 'center', opacity: isArchived ? 0.55 : 1 }}>
      ...
      <View style={{ gap: 6, alignItems: 'flex-end' }}>
        ...
        {isArchived ? (
          <PressableScale onPress={() => { feedback.tap(); onSetStatus('draft'); }} disabled={busy} accessibilityRole="button" accessibilityLabel={`Restore ${meal.title} to draft`} hitSlop={6} style={{ paddingHorizontal: 12, height: 32, borderRadius: 10, backgroundColor: '#252a34', alignItems: 'center', justifyContent: 'center', opacity: busy ? 0.6 : 1 }}>
            <Text style={{ fontFamily: Font.semibold, fontSize: 12.5, color: Palette.textMuted }}>Restore</Text>
          </PressableScale>
        ) : isLive ? (
          <PressableScale onPress={() => { feedback.tap(); onSetStatus('paused'); }} disabled={busy} accessibilityRole="button" accessibilityLabel={`Pause ${meal.title}`} hitSlop={6} style={{ paddingHorizontal: 12, height: 32, borderRadius: 10, backgroundColor: '#252a34', alignItems: 'center', justifyContent: 'center', opacity: busy ? 0.6 : 1 }}>
            <Text style={{ fontFamily: Font.semibold, fontSize: 12.5, color: Palette.textMuted }}>Pause</Text>
          </PressableScale>
        ) : (
          <View style={{ flexDirection: 'row', gap: 6 }}>
            <PressableScale onPress={() => { feedback.tap(); onSetStatus('published'); }} disabled={busy} accessibilityRole="button" accessibilityLabel={`Publish ${meal.title}`} hitSlop={6} style={{ paddingHorizontal: 10, height: 32, borderRadius: 10, backgroundColor: ORANGE, alignItems: 'center', justifyContent: 'center', opacity: busy ? 0.6 : 1 }}>
              <Text style={{ fontFamily: Font.semibold, fontSize: 12.5, color: '#fff' }}>Publish</Text>
            </PressableScale>
            <PressableScale onPress={() => { feedback.tap(); onSetStatus('archived'); }} disabled={busy} accessibilityRole="button" accessibilityLabel={`Archive ${meal.title}`} hitSlop={6} style={{ paddingHorizontal: 10, height: 32, borderRadius: 10, backgroundColor: '#1d2129', borderWidth: 1, borderColor: '#3f4451', alignItems: 'center', justifyContent: 'center', opacity: busy ? 0.6 : 1 }}>
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        {!prepperId ? (
          ...
          <View style={{ padding: 20, gap: 12 }}>
            {[0, 1, 2, 3].map(i => (
              <Skeleton key={i} width="100%" height={72} radius={16} style={{ backgroundColor: '#ffffff18' }} />
            ))}
          </View>
        ) : isError ? (
          <MotiView from={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'timing', duration: 260 }} style={{ flex: 1, alignItems: 'center', justifyContent: 'center', padding: 32, gap: 12 }}>
            ...
            <Text style={{ fontFamily: Font.heading, fontSize: 16, color: '#fff' }}>couldn&apos;t load your menu</Text>
            ...
            <PressableScale onPress={() => { feedback.tap(); void refetch(); }} accessibilityRole="button" accessibilityLabel="Retry loading menu" style={{ marginTop: 4, paddingHorizontal: 20, height: 48, borderRadius: Radius.pill, backgroundColor: ORANGE, alignItems: 'center', justifyContent: 'center' }}>
