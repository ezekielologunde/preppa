...
export default function CartScreen() {
  ...
  function keepOnly(keepKey: string) {
    ...
    removeItems.mutate(drop, { onSuccess: () => feedback.success(), onError: () => feedback.error() });
  }
        ...
        <MotiView key={it.id} from={{ opacity: 0, translateX: -8 }} animate={{ opacity: 1, translateX: 0 }} transition={{ type: 'timing', duration: 260, delay: i * 50 }}>
          <View style={{ backgroundColor: Palette.surface, borderRadius: Radius.md, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 12 }}>
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: Palette.canvas, borderRadius: Radius.pill, paddingHorizontal: 6, paddingVertical: 4 }}>
              <PressableScale onPress={() => { feedback.tap(); updateItem.mutate({ itemId: it.id, quantity: it.quantity - 1 }, { onSuccess: () => feedback.success(), onError: () => feedback.error() }); }} accessibilityRole="button" accessibilityLabel="Decrease quantity" hitSlop={8} style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: Palette.surface, alignItems: 'center', justifyContent: 'center' }}>
              ...
              <PressableScale onPress={() => { feedback.tap(); updateItem.mutate({ itemId: it.id, quantity: it.quantity + 1 }, { onSuccess: () => feedback.success(), onError: () => feedback.error() }); }} accessibilityRole="button" accessibilityLabel="Increase quantity" hitSlop={8} style={{ width: 28, height: 28, borderRadius: 14, backgroundColor: ORANGE, alignItems: 'center', justifyContent: 'center' }}>
      ...
      <SafeAreaView edges={['top']} style={{ flex: 1 }}>
        ...
        {!user ? (
          ...
          <MotiView from={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'timing', duration: 260 }}
            ...
            <PressableScale onPress={() => { feedback.tap(); void refetch(); }} accessibilityRole="button" accessibilityLabel="Retry loading cart"
