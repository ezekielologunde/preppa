...
export function CuisineCard({ cuisine, onPress }: { cuisine: Cuisine; onPress?: () => void }) {
  return (
    <PressableScale onPress={() => { feedback.tap(); onPress?.(); }} accessibilityRole="button" style={{ width: 150, height: 150, borderRadius: 20, overflow: 'hidden', backgroundColor: '#eee' }} accessibilityLabel={`${cuisine.name} cuisine, ${cuisine.meals} meals`}>
