...
function percentileLabel(pct: number): { text: string; color: string } | null {
  ...
  return (
    <View style={{ backgroundColor: '#FFFFFF', borderRadius: 18, padding: 20, borderLeftWidth: 3, borderLeftColor: Palette.brand, gap: 14, shadowColor: '#1A1714', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.06, shadowRadius: 6, elevation: 2 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <View style={{ backgroundColor: Palette.brand + '22', borderRadius: Radius.pill, paddingHorizontal: 10, paddingVertical: 4 }}>
          <Text style={{ fontFamily: Font.semibold, fontSize: 11.5, color: Palette.brand }}>this week</Text>
        </View>
      </View>
      ...
      {isLoading || !data ? (
        ...
        <View style={{ gap: 12 }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <Text style={{ fontFamily: Font.display, fontSize: 22, color: Palette.brand, letterSpacing: -0.5, fontVariant: ['tabular-nums'] }}>
            ...
            {(() => {
              ...
              return (
                <View style={{ backgroundColor: label.color + '22', borderRadius: Radius.pill, paddingHorizontal: 10, paddingVertical: 4 }}>
                  <Text style={{ fontFamily: Font.semibold, fontSize: 12, color: label.color }}>{label.text}</Text>
                </View>
              );
            })()}
          </View>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
            <Text style={{ fontFamily: Font.semibold, fontSize: 14, color: INK }}>
              ${(data.weeklyRevenue ?? 0).toFixed(0)} this week
            </Text>
            <Text style={{ fontFamily: Font.body, fontSize: 13, color: MUTED }}> · </Text>
            <Text style={{ fontFamily: Font.semibold, fontSize: 14, color: MUTED }}>
              {data.repeatRate ?? 0}% repeat customers
            </Text>
          </View>

          {data.streak >= 2 ? (
            <Text style={{ fontFamily: Font.semibold, fontSize: 14, color: INK }}>
              {data.streak}-day streak!{data.streak >= 7 ? ' — keep it up!' : ''}
            </Text>
          ) : null}
          ...
          >
            <Text style={{ fontFamily: Font.semibold, fontSize: 13.5, color: Palette.brand }}>Full analytics</Text>
