-- 0135_fix_create_order_correct_schema.sql
--
-- The create_order RPC accumulated incorrect column references across several
-- patch migrations (0109, 0111, 0134). It was inserting into columns named
-- 'fulfillment' and 'note' which do not exist; the actual table columns are
-- 'fulfillment_type' and 'fulfillment_note'. It also queried cart_items via
-- ci.user_id (no such column — cart_items links via cart_id) and used m.price
-- (the column is base_price). This migration drops both broken overloads and
-- replaces them with a single correct 8-param function that also adds the
-- missing p_scheduled_at parameter.

-- Drop both broken overloads
DROP FUNCTION IF EXISTS public.create_order(
  fulfillment_type, uuid, text, numeric, text, numeric
);
DROP FUNCTION IF EXISTS public.create_order(
  fulfillment_type, uuid, text, numeric, text, numeric, uuid
);
